def _maximal_subarray(arr, i, j):
    ...
    c = (i + j) // 2
    left, lefti, leftj = _maximal_subarray(arr, i, c)
    right, righti, rightj = _maximal_subarray(arr, c, j)
    crossi, crossj = c, c
    curri, currj = c - 1, c
...
def maximal_subarray(arr):
    return _maximal_subarray(arr, 0, len(arr))


if __name__ == '__main__':
    arr = [-9, 3, 7, 8, -10, 3, 8, 15, 4, -38]
    print(maximal_subarray(arr))
